...
internal sealed class ExecutableReplacer : IExecutableReplacer
{
    ...
    public string Replace(string currentPath, string stagedPath)
    {
        ...
        var retired = current + RetiredSuffix + DateTime.UtcNow.ToString("yyyyMMddHHmmssfff", System.Globalization.CultureInfo.InvariantCulture);
        File.Move(current, retired);
        try { File.Move(staged, current); }
        catch { File.Move(retired, current); throw; }
        return retired;
    }
    ...
    public int RemoveRetiredCopies(string currentPath)
    {
        ...
        foreach (var candidate in Directory.EnumerateFiles(directory, prefix + "*").Where(p => IsRetiredName(Path.GetFileName(p), prefix)))
        {
            try { File.Delete(candidate); removed++; } catch (IOException) { } catch (UnauthorizedAccessException) { }
        }
        return removed;
    }
    ...
    private static string RequireFullPath(string value, string parameterName)
    {
        ...
        if (!Path.IsPathFullyQualified(value)) throw new ArgumentException("The path must be fully qualified.", parameterName);
